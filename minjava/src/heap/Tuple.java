...
/** get the length of a tuple, call this method if you did 
  ...
  */
  ...
  public Tuple setScore(float score)
  	throws IOException, FieldNumberOutOfBoundException
  {
	  int lastFldNo = fldOffset.length-1;
	  if ( (lastFldNo > 0) && (lastFldNo <= fldCnt))
	    {
	     Convert.setFloValue (score, fldOffset[lastFldNo -1], data);
	     return this;
	    }
	    else  
	     throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND"); 
  ...
  public float getScore() 
		...
		int lastFldNo = fldOffset.length-1;
	      if ( (lastFldNo > 0) && (lastFldNo <= fldCnt))
	       {
	        val = Convert.getFloValue(fldOffset[lastFldNo -1], data);
	        return val;
	       }
	      else 
	       throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
 ...
 throws IOException, InvalidTypeException, InvalidTupleSizeException		
{
  ...
  //sets last attribute of the tuple
