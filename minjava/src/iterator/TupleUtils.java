...
public class TupleUtils {
	 ...
	 *         tuple is smaller,
	 */
	public static int CompareTupleWithTuple(AttrType fldType, Tuple t1, int t1_fld_no, Tuple t2, int t2_fld_no) throws IOException, UnknowAttrType,
			TupleUtilsException {
		int t1_i, t2_i;
		float t1_r, t2_r;
		String t1_s, t2_s;
	 ...
	 *                exception from this class
	 */
	public static int CompareTupleWithValue(AttrType fldType, Tuple t1, int t1_fld_no, Tuple value) throws IOException, UnknowAttrType, TupleUtilsException {
	 ...
	 */
	// changed by USSSSS
	public static boolean Equal(Tuple t1, Tuple t2, AttrType types[], int len) throws IOException, UnknowAttrType, TupleUtilsException {
		int i;
	 ...
	 *                exception from this class
	 */
	public static void SetValue(Tuple value, Tuple tuple, int fld_no, AttrType fldType) throws IOException, UnknowAttrType, TupleUtilsException {
	 ...
	 *                exception from this class
	 */
	public static short[] setup_op_tuple(Tuple Jtuple, AttrType[] res_attrs, AttrType in1[], int len_in1, AttrType in2[], int len_in2, short t1_str_sizes[],
			short t2_str_sizes[], FldSpec proj_list[], int nOutFlds) throws IOException, TupleUtilsException {

		// changed by US: assuming that score field is passed as a parameter in
		// this function
		// we decrease the account of res_attrs and nOutFlds
	...
	/**
	 * Assume that the input two tuples, proj_list doesn't include score and
	 * this will modify jtype adding score field and return str_size
	 * 
	 * @param Jtuple
	 * @param res_attrs
	 * @param in1
	...
	public static short[] setup_op_tuple(Tuple Jtuple, AttrType[] res_attrs, AttrType in1[], int len_in1, AttrType in2[], int len_in2, short t1_str_sizes[],
			short t2_str_sizes[], FldSpec proj_list[], int nOutFlds, boolean flag) throws IOException, TupleUtilsException {
		...
		for (i = 0; i < nOutFlds; i++) {
			...
			/*
			 * if (proj_list[i].relation.key == RelSpec.outer) res_attrs[i] =
			 * new AttrType(in1[proj_list[i].offset-1].attrType); else if
			 * (proj_list[i].relation.key == RelSpec.innerRel) res_attrs[i] =
			 * new AttrType(in2[proj_list[i].offset-1].attrType);
			 */
		...
		res_attrs[i] = new AttrType(AttrType.attrReal);
		...
		try {
			Jtuple.setHdr((short) (nOutFlds + 1), res_attrs, res_str_sizes);
		} catch (Exception e) {
			throw new TupleUtilsException(e, "setHdr() failed");
		}
		return res_str_sizes;
	...
	public static short[] setup_op_tuple(Tuple Jtuple, AttrType res_attrs[], AttrType in1[], int len_in1, short t1_str_sizes[], FldSpec proj_list[],
			int nOutFlds) throws IOException, TupleUtilsException, InvalidRelation {
		short[] sizesT1 = new short[len_in1];
		int i, count = 0;
